...
class DataModule(LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        self.test_imgs, test_meta = get_val_fnames(self.args, self.data_path, meta)
        ...
        if self.args.exec_mode != "predict" or self.args.benchmark:
            ...
            self.train_imgs_all, self.train_lbls_all = imgs,lbls

            print0(
                f"{len(self.train_imgs)} training, {len(self.val_imgs)} validation, {len(self.test_imgs)} test examples")
        else:
            # self.kwargs.update({"meta": meta})

            self.kwargs.update({"meta": test_meta})
            print0(f"{len(self.train_imgs)} training, {len(self.val_imgs)} validation,{len(self.test_imgs)}")

    def train_dataloader(self):

        return fetch_dali_loader(self.train_imgs, self.train_lbls ,self.args.batch_size, "train", **self.kwargs)
        # return fetch_dali_loader(self.train_imgs_all, self.train_lbls_all, self.args.batch_size, "train", **self.kwargs)

    def val_dataloader(self):
        return fetch_dali_loader(self.val_imgs, self.val_lbls, 1, "eval", **self.kwargs)
        # return fetch_dali_loader(self.train_imgs_all, self.train_lbls_all, 1, "eval", **self.kwargs)
    ...
    def test_dataloader(self):
        ...
        # return fetch_dali_loader(self.val_imgs, None, 1, "test", **self.kwargs)
...
def get_val_fnames(args,data_path,meta=None):
    kfold = get_kfold_splitter(args.nfolds)
    imgs = load_data(data_path, "*_x.npy",non_empty=False)
    train_idx, val_idx = list(kfold.split(imgs))[args.fold]
    val_imgs = sorted(get_split(imgs, val_idx))

    if meta is not None:
        meta = sorted(get_split(meta, val_idx))

    return val_imgs,meta

...
def test():
    path = '/home/user/4TB/Chenbonian/medic-segmention/BraTS_dataset/BraTS2023_train/labels'
    inferts_path = '/home/user/4TB/Chenbonian/medic-segmention/isnet/results/new-bast-last-v33-1000-500-0.7-0.3-0.4-tta-2023'
    ...
    for idx, file_name in enumerate(infer_files):
        if substring in str(file_name):
            ...
            # fn = file_name.split(".")[-3]
            pred = file_name.split("/")[-1]
            # print(pred)
            name = pred.split("_")
            print(name)
            label_path = os.path.join(path, name[0] +'_seg'+'.nii.gz')

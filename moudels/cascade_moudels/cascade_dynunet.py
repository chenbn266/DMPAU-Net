...
class DynUNet(nn.Module):
    ...
    Args:
        ...
        deep_supervision: whether to add deep supervision head before output. Defaults to ``False``.
            If ``True``, in training cascade_moudels, the forward function will output not only the last feature
    ...
    def __init__(
        ...
        self.input_block_2 = self.get_input_block_2()
        ...
        self.skip_layers_2 = create_skips(
            0,
            [self.input_block_2] + list(self.downsamples),
            self.upsamples[::-1],
            self.deep_supervision_heads,
            self.bottleneck,
        )
    ...
    def forward(self, x):
        ...
        out2 = self.skip_layers_2(out1)
        out2 = self.output_block(out2)
        # print("out4.shape", out2.shape)

        if self.training and self.deep_supervision:
            out_all = [out2]
            feature_maps = self.heads[1 : self.deep_supr_num + 1]
            for feature_map in feature_maps:
                out_all.append(interpolate(feature_map, out2.shape[2:]))
